...
float sdSoftBox2D(vec2 p, vec2 b, vec4 r) {
...
float sdHorseshoe2D(vec2 p, float angle, float r, float le, float th) {
...
float sdExtrudeChamfer(float d2d, float pz, float h, float r) {
    vec2 w = vec2(d2d + r, abs(pz) - (h - r));
